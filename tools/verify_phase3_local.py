...
checks. The script never installs toolchains or packages: an acceptance run
fails if a required local toolchain/target/cargo-fuzz installation is absent.
"""
...
class Runner:
    ...
    def record_static(self, name: str, detail: str) -> None:
        self.records.append(
            {"name": name, "command": None, "status": "pass", "seconds": 0.0, "detail": detail}
        )
    ...
    def run(self, name: str, command: list[str], *, capture: bool = False) -> str:
        ...
        if completed.returncode != 0:
            ...
            raise VerificationFailure(f"{name} failed with exit status {completed.returncode}")
        if capture and completed.stdout:
            print(completed.stdout, end="")
    ...
    def write_report(self, mode: str, offline: bool, skips: list[str]) -> None:
        ...
        dirty = bool(_capture_optional(["git", "status", "--porcelain"]))
        payload = {
...
def verify_wiring(runner: Runner) -> None:
    ...
    missing = [name for name in required if f'include!("bounded_end_to_end_candidate/{name}")' not in text]
    if missing:
        raise VerificationFailure(f"Experiment 0179 files are not wired: {', '.join(missing)}")
    ...
    runner.record_static("Experiment 0179 wiring", f"{len(required)} implementation/test files wired")
...
def run_model(runner: Runner, campaigns: int, steps: int) -> None:
    ...
    runner.run(
        ...
        [sys.executable, str(model), "--campaigns", str(campaigns), "--steps", str(steps)],
    )


def run_quick(runner: Runner) -> None:
    runner.run("Locked dependency graph", ["cargo", "metadata", "--locked", "--no-deps", "--format-version", "1"])
    runner.run("Rust formatting", ["cargo", "fmt", "--all", "--", "--check"])
    runner.run(
        "Phase 3 Clippy",
        ["cargo", "clippy", "--locked", "-p", "ucof-experiments", "--all-targets", "--", "-D", "warnings"],
    )
    runner.run(
        "Phase 3 Rust tests",
        ["cargo", "test", "--locked", "-p", "ucof-experiments", "--all-targets"],
    )
    runner.run("Independent Phase 3 models", [sys.executable, "tools/validate_phase3_models.py"])
    runner.run(
        "Pinned EXP-0002 invalid corpus",
        [sys.executable, "tools/exp0002_invalid_vectors.py", "--verify-vectors", "tests/vectors/exp-0002-invalid"],
    )
    runner.run(
...
def run_http_and_policy(runner: Runner) -> None:
    ...
    runner.run("Policy reward decomposition", [sys.executable, "tools/verify_exp0003_policy_reward_decomposition.py"])
    runner.run(
...
def run_msrv_and_portability(runner: Runner) -> None:
    require_command(["cargo", "+1.85.0", "--version"], "Rust 1.85.0 toolchain")
    runner.run(
    ...
    for target in ("i686-unknown-linux-gnu", "powerpc64-unknown-linux-gnu"):
        if target not in installed_targets:
            raise VerificationFailure(f"required portability target is not installed: {target}")
        runner.run(
...
def run_fuzz_smoke(runner: Runner) -> None:
    ...
    runner.run("Fuzz formatting", ["cargo", "+nightly", "fmt", "--manifest-path", "fuzz/Cargo.toml", "--", "--check"])
    runner.run("Compile fuzz targets", ["cargo", "+nightly", "fuzz", "build"])
    targets_text = runner.run("List fuzz targets", ["cargo", "+nightly", "fuzz", "list"], capture=True)
    ...
    for target in targets:
        runner.run(
            f"Fuzz smoke {target}",
            ["cargo", "+nightly", "fuzz", "run", target, "--", "-runs=256", "-max_len=65536"],
        )
...
def run_acceptance(runner: Runner, skip_fuzz: bool, skips: list[str]) -> None:
    ...
    runner.run("Workspace Rust tests", ["cargo", "test", "--locked", "--workspace", "--all-targets"])
    runner.run("Rust documentation tests", ["cargo", "test", "--locked", "--workspace", "--doc"])
    run_http_and_policy(runner)
    run_msrv_and_portability(runner)
    if skip_fuzz:
        skips.append("fuzz-smoke")
        print("SKIP fuzz smoke (--skip-fuzz); report is not a complete acceptance run")
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--acceptance", action="store_true", help="run the full former CI acceptance surface locally")
    parser.add_argument("--model-only", action="store_true", help="run only wiring and the independent 0179 model")
    parser.add_argument("--offline", action="store_true", help="set CARGO_NET_OFFLINE=true for Cargo commands")
    parser.add_argument("--skip-fuzz", action="store_true", help="development escape hatch; makes --acceptance incomplete")
    parser.add_argument("--campaigns", type=int, default=128, help="independent-model randomized campaigns")
    parser.add_argument("--steps", type=int, default=128, help="steps per independent-model campaign")
