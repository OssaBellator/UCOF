...
fuzz_target!(|data: &[u8]| {
    ...
    let replacement_id = u64::try_from((replacement_index + 1) * 2).expect("replacement id");
    let replacement_seed = data.get(count + 4).copied().unwrap_or(199);
    let mut operations = vec![
        ImmutableBatchOperation::Delete(delete_id),
        ImmutableBatchOperation::Put(object(replacement_id, replacement_seed, payload_len)),
    ];
    if data.get(count + 5).is_some_and(|byte| byte & 1 != 0) {
        let insertion_index = data
            .get(count + 6)
            .map_or(count / 2, |byte| usize::from(*byte) % (count + 1));
        let insertion_id = u64::try_from(insertion_index * 2 + 1).expect("bounded insertion id");
    ...
    assert_eq!(
        report.path_local_final_leaf_counts.iter().sum::<usize>(),
        expected_objects
    );
    assert_eq!(
        report.canonical_final_leaf_counts.iter().sum::<usize>(),
        expected_objects
    );
    assert_eq!(
    ...
    assert_eq!(
        report.exact_leaf_layout_match,
        report.first_differing_leaf.is_none()
    );
    if report.exact_leaf_layout_match {
        assert_eq!(
            report.path_local_final_leaf_counts,
            report.canonical_final_leaf_counts
        );
    }
    ...
    let validated =
        validate_canonical_occupancy(&result.bytes, limits).expect("canonical mixed output");
