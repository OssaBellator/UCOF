...
use ucof_experiments::{enumerate_previous_chain_at, Exp0002SourceChainLimits};
...
fuzz_target!(|data: &[u8]| {
    ...
    let mut source = Exp0002SliceSource::new(data);
    let _ = enumerate_previous_chain_at(
        &mut source,
        &Exp0002SourceChainLimits {
            candidate: source_limits,
            max_chain_depth: 16,
            max_total_bytes_read: file_len.saturating_mul(32).saturating_add(64 * 1024),
        },
    );
});
