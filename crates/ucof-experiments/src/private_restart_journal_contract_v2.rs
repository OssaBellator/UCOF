...
impl RestartJournal {
    ...
    fn authority(&self) -> RecoveryAuthority {
        match self.state {
            JournalState::Prepared | JournalState::Constructing | JournalState::PrivateSynced => {
...
fn validate_artifacts(
    ...
    for (index, artifact) in artifacts.iter().enumerate() {
        if artifacts[..index].iter().any(|existing| {
            existing.kind == artifact.kind && existing.segment_id == artifact.segment_id
        }) {
