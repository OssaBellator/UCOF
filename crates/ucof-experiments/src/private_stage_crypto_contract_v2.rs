...
fn u64_field(bytes: &[u8], offset: usize) -> Result<u64, CryptoContractError> {
    ...
    Ok(u64::from_le_bytes(
        field
            .try_into()
            .map_err(|_| CryptoContractError::Invalid("integer field"))?,
    ))
}

fn decode_header(
    record: &[u8],
) -> Result<(RecordIdentity, u64, [u8; 12]), CryptoContractError> {
    let header = record
        .get(..HEADER_LEN)
...
mod tests {
    ...
    fn envelope_round_trip_binds_all_identity_fields_and_length() {
        ...
        assert_eq!(open_record(&record, expected, &aead).expect("open"), plaintext);
    ...
    fn reorder_and_duplicate_records_require_expected_sequence() {
        ...
        let first = seal_record(&mut context, StageKind::SortRun, 17, 0, b"first", &aead)
            .expect("first");
        let second = seal_record(&mut context, StageKind::SortRun, 17, 1, b"second", &aead)
            .expect("second");
        ...
        assert_eq!(
            open_record(&second, identity(StageKind::SortRun, 17, 1), &aead)
                .expect("second open"),
            b"second"
        );
    }
