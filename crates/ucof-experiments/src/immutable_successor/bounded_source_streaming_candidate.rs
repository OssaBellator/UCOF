...
mod bounded_source_streaming_candidate_tests {
    ...
    struct StagePathGuard(Option<PathBuf>);
    ...
    impl StagePathGuard {
        ...
        fn path(&self) -> &Path {
            self.0.as_deref().expect("armed stage path")
        }

        fn disarm(mut self) -> PathBuf {
            self.0.take().expect("armed stage path")
        }
    }
    ...
    impl<S: ImmutableStreamingPayloadSource> Iterator for DescriptorRecords<'_, S> {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            let result = (|| {
                ...
                *self.object_bytes = (*self.object_bytes)
                ...
                *self.version_checks = (*self.version_checks)
                    .checked_add(1)
    ...
    fn create_stage(directory: &Path) -> Result<(StagePathGuard, File), CandidateError> {
        ...
        let file = options
            .open(&path)
            .map_err(|error| CandidateError::StageIo(error.kind()))?;
        Ok((StagePathGuard::new(path), file))
    }
    ...
    fn prepare_source_descriptors<S: ImmutableStreamingPayloadSource>(
        ...
        let (stage_guard, file) = create_stage(directory)?;
        ...
        let spill = sorted.map_err(CandidateError::Spill)?;
        writer
            .flush()
            .map_err(|error| CandidateError::StageIo(error.kind()))?;
        let retained = writer
    ...
    #[test]
    fn preflight_failures_leave_output_and_stage_directory_untouched() {
        let duplicate_directory = TestDirectory::new("duplicate");
        let mut duplicate_sources = [
            CandidateSource::new(2, vec![1; 10]),
            CandidateSource::new(2, vec![2; 10]),
        ];
        let mut duplicate_output = Vec::new();
        let duplicate_error = write_genesis_sources_bounded_candidate(
            &mut duplicate_output,
            &mut duplicate_sources,
            &duplicate_directory.0,
        ...
        assert!(fs::read_dir(&duplicate_directory.0).unwrap().next().is_none());

        let metadata_directory = TestDirectory::new("metadata-failure");
        let mut metadata_sources = [
            CandidateSource::new(2, vec![2; 10]),
            CandidateSource::new(1, vec![1; 10]),
            CandidateSource::new(3, vec![3; 10]),
        ];
        metadata_sources[2].fail_version = true;
        let mut metadata_output = Vec::new();
        let metadata_error = write_genesis_sources_bounded_candidate(
            &mut metadata_output,
        ...
        assert!(fs::read_dir(&metadata_directory.0).unwrap().next().is_none());

        let size_directory = TestDirectory::new("output-limit");
        let mut size_sources = [CandidateSource::new(1, vec![1; 10])];
        let mut tiny_limits = ImmutableLimits::default();
        tiny_limits.max_output_bytes = 1;
        let mut size_output = Vec::new();
        let size_error = write_genesis_sources_bounded_candidate(
            &mut size_output,
            &mut size_sources,
            &size_directory.0,
            ImmutableSourceStreamingWriteOptions::default(),
        ...
        assert!(matches!(
            ...
            CandidateError::Writer(ImmutableSourceStreamingWriteError::Format(
                ImmutableError::Limit("output")
            ))
        ));
        assert!(size_output.is_empty());
        assert!(fs::read_dir(&size_directory.0).unwrap().next().is_none());
    }
}
