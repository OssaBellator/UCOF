fn prepare_source_bound_encrypted_tree_restart_continuation(
    ...
    let fresh_lease_size = object_nonce_count
        ...
        .ok_or_else(|| "fresh source-bound restart lease size".to_owned())?;
    ...
    if fresh_session.remaining() != tree_nonce_count {
        return Err("fresh source-bound restart lease remainder".into());
    }
...
fn continue_source_bound_encrypted_tree_restart<W, S>(
    journal: &LinuxDurableNonceJournal,
    stage_directory_path: &Path,
    work_directory: &Path,
    writer: &mut W,
...
{
    let prepared = prepare_source_bound_encrypted_tree_restart_continuation(
        journal,
        stage_directory_path,
        work_directory,
        sources.len(),
