# UCOF Format Change Proposals

This directory contains normative design proposals and their permanent decision history.

Read [the proposal process](../PROPOSAL_PROCESS.md) before submitting an FCP.

## Proposal index

| FCP | Title | Status | Core/profile impact |
|---|---|---|---|
| [FCP-0001](0001-exp-0001-framing.md) | UCOF-EXP-0001 minimal framing experiment | Review | Disposable core framing experiment |

## Rules

- Use the next unused four-digit number.
- Never reuse a number, including after withdrawal or rejection.
- Begin from [TEMPLATE.md](TEMPLATE.md).
- Keep rejected, withdrawn, deferred, and superseded records.
- Do not assign permanent registry values until the defining FCP is accepted.
- Link implementation pull requests and conformance vectors, but keep normative requirements in the proposal or specification.
- Update this index when proposal status changes.

The current proposal allocates only an experimental epoch. It does not allocate permanent registry identifiers or stabilize any wire behavior.
