...
This guidance remains outside core byte validity. EXP-0003 may expose authenticated sequence, snapshot, commit, and ancestry evidence, but the format cannot decide who is authorized to establish or advance application trust.
...
Diagnostic, salvage, or recovery commands must never update the trusted checkpoint implicitly.
...
This guidance defines the application decision boundary. UCOF bytes can provide authenticated sequence and ancestry evidence, but cannot decide who is authorized to establish or change local trust.
