...
| Bounded canonical output | #35, #36 | Canonical genesis bytes through bounded sequential sinks from owned and strongly versioned payload sources, with incremental hashing and version-change failure | Atomic staging integration and bounded remote inventory |
| Active and selected active rewrites | #38, #39 | Strict active-file validation, borrowed authenticated payload ranges, inactive-history skipping, caller-selected output, exact selected read accounting, and untouched-sink selection failures | Bounded remote source implementation and selected historical-state streaming |
...
- Output chain: #23 → #35 → #36 → #38 → #39 → #41.
...
**Advanced:** Strict validation, lookup, history enumeration, report-only recovery, rewrite-all, selected rewrite, selected historical-state retention, and reproducible operation accounting exist for slices and bounded sources. Canonical output now accepts strongly versioned payload sources. Strict active-file adapters borrow authenticated payload ranges and skip inactive history; selected adapters read only retained active payloads.

**Open:** Build a strictly validated active inventory directly over `ImmutableReadAt`/stable remote sources without first materializing the complete file. Stream selected historical states to canonical output. Add maintained authenticated HTTP/cloud adapters, provider-specific retry classification, real waits, jitter/authentication policy, native asynchronous cancellation, and realistic provider latency/billing/cache/concurrency measurements.
...
**Open:** Obtain application-profile adoption and root-selection policy. Pin cross-language rewrite bytes for profile-defined cases. Define extension preservation, provenance and signature reissuance, large-graph spill, and historical-state semantic compaction.

### 5. Vectors and fuzzing

**Advanced:** Valid, invalid, interrupted, fork, recovery, source, history, rewrite, occupancy, persistent replacement/insertion/deletion/multi-`Put`/mixed, compaction, retry, spill, active streaming, selected streaming, and semantic-streaming evidence exists. PR #40 adds independently generated mixed transition bytes rather than treating Rust output as the oracle.

**Open:** Produce a complete cross-language corpus for the eventually selected epoch geometry, including all structural transition boundaries. Add provider-specific retry/source traces, fresh-process restart and physical power-loss corpora, and hostile remote source-to-sink traces.
...
**Open:** Integrate the guidance into applications, user interfaces, and durable checkpoint stores. Define application-specific authorization and trust-reset procedures.
