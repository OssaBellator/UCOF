...
| Complete append-tail writer stack | #47, #54, #57, #58, #59, #60 | Byte-identical bounded base-plus-tail output for replacement, insertion, deletion, shared multi-`Put`, and canonical mixed batches behind one borrowed-payload dispatcher | Source-backed mutation planning and private atomic staging/publication |
| Bounded and version-bound base copying | #61, #62 | Two-pass whole-file length/SHA-256 verification, cumulative read budgets, bounded I/O, tail withholding, and strong non-ABA checks around every length/range operation | Maintained provider adapters and private atomic staging/publication |
...
- Writer chain: #7 → #15 → #17 → #21 → #23 → #27 → #30 → #34 → #37 → #47 → #54 → #57 → #58 → #59 → #60 → #61 → #62.
...
### 2. Persistent writer, tails, and source copying
...
**Open:** Tail construction still starts from in-memory validated mutation planning. Add bounded source-backed planning. Neither #61 nor #62 gives atomic visibility: earlier verified base chunks may remain after a later failure. Compose the stack with private staging, validation, file synchronization, no-overwrite publication, directory synchronization, and explicit indeterminate outcomes. Global minimality and proposed-epoch migration remain open.
...
**Advanced:** Evidence covers strict/recovery/history/source/rewrite paths; occupancy; persistent replacement/insertion/deletion/multi-`Put`/mixed writing; all five tail writers and dispatcher; unversioned and strong-version base copying; compaction; retry and HTTP classification; cooperative waits; spill transitions/restart; and versioned historical output. PRs #40 and #48 provide independent byte or policy oracles.

**Open:** Produce the complete cross-language corpus for the selected epoch geometry, concrete-provider transport traces, authenticated journal and physical power-loss corpora, and hostile maintained-adapter traces.

### 6. Spill and publication

**Advanced:** Models and Unix tests cover bounded resources, private staging, symlink refusal, ownership cleanup, no-overwrite publication, synchronization order, explicit indeterminate outcomes, and injected failures. PR #53 adds conservative authenticated fresh-process classification.

**Open:** Integrate persistent source output with private staging. Implement and qualify the durable authenticated journal, descriptor-relative secure handles, effective-user ownership checks, encrypted framing and nonce management, physical power-loss behavior, network-filesystem policy, and supported platforms.
