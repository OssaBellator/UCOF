...
This ledger separates **implemented research mechanisms** from **production-qualified claims**. A green implementation test can move a mechanism row without automatically moving its platform/operational qualification row.
...
| burned committed ranges are not reused | 0174, 0177, 0179 retry model | evidence | physical filesystem qualification |
...
| restart after metadata checkpoint compaction | 0179 pending | partial | full CI acceptance; platform qualification |
...
| checkpoint/protected metadata included in cap | 0179 pending | partial | CI acceptance; full lifecycle integration decision |
...
| append-only metadata reclamation | 0179 pending | partial | CI acceptance; retention/cadence policy |
...
| Requirement | Current evidence | Status | Remaining qualification |
|---|---|---:|---|
| ciphertext corruption/truncation/substitution | AEAD/HMAC tests across 0170–0179 | evidence | broader campaign optional |
| nonce uniqueness/no-wrap | lease model + restart tests | evidence | long campaign/platform qualification |
| directory path replacement | procfd descriptor-pinning evidence | evidence | same-UID final-step race remains |
| staged-name replacement | detected at classification/final checks in publication/retirement paths | partial | compaction cleanup inherits same final-step race |
...
These claims remain **external/open** even if Experiment 0179 becomes fully green:
...
Issue #11 should not close merely because all mechanism rows are `evidence`.
