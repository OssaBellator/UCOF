...
| ordinary stage-manifest / ordinary retirement primitive write capacity | shared journal metadata-slot guard is enforced on ordinary manifest/retirement `create_new` paths; full-journal rejection and exact-one-slot success regressions are wired | partial | complete pinned local acceptance; filesystem reservation/concurrent-consumption policy remains external |
